...
std::string serializeArtifact(const StageArtifact& a) {
    ...
    os.write(reinterpret_cast<const char*>(a.spirv.data()), static_cast<std::streamsize>(a.spirv.size() * 4));
...
bool deserializeArtifact(const std::string& data, StageArtifact& out) {
    try {
        ...
        is.read(reinterpret_cast<char*>(out.spirv.data()), static_cast<std::streamsize>(spirv_count * 4));
